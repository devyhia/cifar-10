...
def feature_extract(pth):
    r_image = cv2.resize(pth,None,fx=4, fy=4, interpolation = cv2.INTER_CUBIC)
...
svc = svm.SVC(kernel='poly', max_iter=2)
svc.fit(x, y)

count = 0
X_pred = svc.predict(x)
print metrics.accuracy_score(y, X_pred)                       # Accuracy = 12%
